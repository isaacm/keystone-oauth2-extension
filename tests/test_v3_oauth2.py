...
class ConsumerCRUDTests(OAuth2BaseTests):
    ...
    def test_consumer_update(self):
        ...
        update_response = self.patch(
            self.CONSUMER_URL + '/%s' % original_id, body=body)
...
class AccessTokenEndpointTests(OAuth2BaseTests):
    ...
    @staticmethod
    def new_access_token_ref(user_id, consumer_id):
        token_ref = {
            'id': uuid.uuid4().hex,
...
class OAuth2FlowBaseTests(OAuth2BaseTests):
    ...
    def _flow_step_request_authorization(
    ...
    def _flow_step_grant_authorization(self, response, scopes,
                                       expected_status=302, **kwargs):
        # POST authorization url to simulate ResourceOwner
    ...
    def _generate_urlencoded_request(self, authorization_code,
        ...
        body = 'grant_type=authorization_code&code=%s&redirect_uri=%s' % authorization_code, \
               self.DEFAULT_REDIRECT_URIS[0]
        headers = {
    ...
    def _flowstep_obtain_access_token(self, response, expected_status=200):
        authorization_code = self._extract_authorization_code_from_header(
            response)

        consumer_id = self.consumer['id']
        consumer_secret = self.consumer['secret']

        headers, body = self._generate_json_request(
            authorization_code, consumer_id, consumer_secret)
        # POST to the token url
        return self.post(
            '/OS-OAUTH2/access_token',
            body=body,
            headers=headers,
            expected_status=expected_status)
...
class OAuth2AuthorizationCodeFlowTests(OAuth2FlowBaseTests):
    ...
    def test_flow_step_grant_authorization(self):
        expected_redirect_uri = self.DEFAULT_REDIRECT_URIS[0]
        expected_scopes = self.DEFAULT_SCOPES
    ...
    def test_granting_authorization_by_different_user_fails(self):
        ...
        # First, request authorization for our user
        expected_redirect_uri = self.DEFAULT_REDIRECT_URIS[0]
        expected_scopes = self.DEFAULT_SCOPES
        get_response = self._flowstep_request_authorization(
            scope=expected_scopes,
...
class OAuth2AccessTokenFromCodeFlowTests(OAuth2FlowBaseTests):
    ...
    def test_access_code_only_one_use(self):
        ...
        response_ok = self._flowstep_obtain_access_token(
            post_response, expected_status=200)

        response_not = self._flowstep_obtain_access_token(
            post_response, expected_status=401)
...
class OAuth2PasswordGrantFlowTests(OAuth2FlowBaseTests):
    ...
    def test(self):
        ...
        self._assert_access_token(
            response, expected_scopes=scope)
